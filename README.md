...
## 🔴 비교 대상은 반드시 `best_spot.pt` (실데이터만 학습한 것)

YOLO 가중치가 여러 개 있는데 **아무거나 쓰면 비교가 기울어집니다.**

| 가중치 | 학습 데이터 | 비교에 쓸 것 |
|---|---|---|
| **`best_spot.pt`** (W5) | **71858 실데이터만** | ✅ **이것** |
| `w8_stage2_real` | 71856 합성 사전학습 + 실데이터 | ❌ |
| `w8b_rehearsal` | 합성·실 혼합 | ❌ |

VLM은 실데이터만 보고 학습합니다. 그러니 YOLO 쪽도 **실데이터만 본 모델**이어야 합니다. W8 계열(합성 증강)을 상대로 세우면 *"증강한 YOLO vs 증강 안 한 VLM"* 이 되어 비교가 성립하지 않습니다.

기준값: **군함 mAP50 0.979** (P 0.951 / R 0.975), 전체 0.961, 추론 2.6ms.
